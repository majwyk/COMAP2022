...
colorlist = plt.cm.coolwarm(np.linspace(0,1,lakeMead.shape[0]))
...
plot1 = plt.scatter(lakeMead_sort[:,0], lakeMead_sort[:,1], label='original values\nwarmer color means more recent', color = colorlist)
plt.text(lakeMead_sort[0,0],lakeMead_sort[0,1],"Start: "+str(lakeMead_sort[0,0])+" feet, "+str(lakeMead_sort[0,1])+" acre-feet")
plt.text(lakeMead_sort[-1,0],lakeMead_sort[-1,1],"End: "+str(lakeMead_sort[-1,0])+" feet, "+str(lakeMead_sort[-1,1])+" acre-feet")
